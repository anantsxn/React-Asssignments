...
class Product extends Component {
    ...
    // cardProduct = (product) => {
    //     return <Cards key={product.sfid} product={product} />;
    // }

    render() {
        const { products } = this.state;
        const listProducts = products.map(product => { return <Cards key={product.sfid} product={product} /> });
        return (
            <div className="plp">{listProducts}</div>
        )
    }
}
